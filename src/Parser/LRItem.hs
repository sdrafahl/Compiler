...
fixedPointAlgorithm terms prods lritem = foldOverLrItems terms prods lritem
...
foldOverLrItems :: Set Terminal -> Set ProductionRule -> Set LRItem -> Set LRItem
foldOverLrItems terminals' prodRules setOfLrItems =
  let (lrItemAndChildren :: [[TokenOrPlaceholder]]) = Data.List.map (\(LRItem a b lookahead) -> dropBeforeContext b) (Data.Set.toList setOfLrItems)
      ...
      (cProductions :: [ProductionRule]) = Data.List.filter (\(ProductionRule (a', _)) -> Data.List.elem (Token (NonTerm a')) lrItemAndChildrenWithC') listOfproductions
...
getFirst' prodRules terminals' =
  let (firstMap :: First) = createFirstMap prodRules terminals'
      ...
      (terminals'' :: Set Terminal) = getFirstForAllChildren nonTermHeads firstMap
  in  (Data.Set.insert (Terminal "eof") terminals'')
