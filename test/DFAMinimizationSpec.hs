...
spec = do
  ...
  describe "minimizeDFA" $ do
    ...
    it "Should minimize DFA (a|b)" $ minimizeDFA ((DFA ["0", "1", "2"] "0" ["1", "2"] [(Transition "0" 'b' "1"), (Transition "0" 'a' "2")])) `shouldBe` ((DFA ["0", "1"] "0" ["1"] [(Transition "0" 'b' "1"), (Transition "0" 'a' "1")]))
    it "Should minimize DFA (a|b)" $ minimizeDFA (DFA ["0", "1"] "0" ["0" ,"1"] [(Transition "0" 'a' "1"), (Transition "1" 'a' "1")]) `shouldBe` (DFA ["0"] "0" ["0"] [(Transition "0" 'a' "0")])
